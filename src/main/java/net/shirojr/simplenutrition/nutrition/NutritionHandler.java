...
public class NutritionHandler {
    ...
    private static void applyEffects(ServerPlayerEntity player, float normalizedNutrition) {
        ...
        if (normalizedNutrition >= 1) {
            ...
            player.addStatusEffect(new StatusEffectInstance(StatusEffects.BLINDNESS, 200, 1, true, false, true), player);
            player.addStatusEffect(new StatusEffectInstance(StatusEffects.NAUSEA, 400, 2, true, false, true), player);
        } else if (normalizedNutrition > 0.8) {
            player.addStatusEffect(new StatusEffectInstance(StatusEffects.NAUSEA, 400, 2, true, false, true), player);
            player.addStatusEffect(new StatusEffectInstance(StatusEffects.BLINDNESS, 100, 1, true, false, true), player);
        } else if (normalizedNutrition > 0.4) {
            player.addStatusEffect(new StatusEffectInstance(StatusEffects.NAUSEA, 200, 1, true, false, true), player);
        }
    }
